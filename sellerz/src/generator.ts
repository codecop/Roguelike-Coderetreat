...
export function generateRandomMap(amountOfMonsters: number = 0): string[] {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      if (Math.random() < 0.05 && !isCornerOfTheMap(x, y, width, height) && currentAmountOfDoors++ < 3) {
        row += '|'; // Door
      }
      else if (Math.random() < 0.1 && isPositionCorrect(x, y, width, height) && currentAmountOfButtons++ < 1) {
          row += 'B'; // Button
     }
        ...
        row += '#'; // Wall
          ...
          if (Math.random() < 0.05 && amountOfMonsters-- > 0) {
          row += 'M'; // Monster
        } else {
          row += ' '; // Empty space
        }
