package org.codecop.rogue.template.client;
...
import org.codecop.rogue.template.client.JsonRestClient;
import org.codecop.rogue.template.client.JsonText;
...
class JsonRestClientTest {
    ...
    void testHelloJsonResponse() {
        wireMockServer.stubFor(get(urlEqualTo("/bar")) //
                .willReturn(aResponse() //
                        .withHeader("Content-Type", "application/json") //
                        .withBody("{\"text\":\"Hello Json\"}")));

        JsonText[] actual = { null };
        client.fetchReleases().subscribe(new Subscriber<JsonText>() {
            @Override
